...
# n_gen: numer of generations, that is the number of evolution of the configurations, it stops when reach this number
n_gen = 1000 # 100-1000 are very low number of generation, just to show how it works
iter_print = n_gen // 10
...
backend_filename = os.path.join(os.path.abspath("./"), "sampler_repar.hdf5")

if load_emcee:
    # sampler = pickle.load(open('sampler.pkl', 'rb'))
    sampler_repar = emcee.backends.HDFBackend(backend_filename, read_only=True)
else:
    backend = emcee.backends.HDFBackend(backend_filename, compression="gzip")
    backend.reset(n_walkers, n_fit_repar)
...
l = "{:12s}: MAP {:10.6f} ".format("M_b", map_Mb_Me_repar)
for j, pc in enumerate(credint):
    l += "HDI@{:.2f}% [{:10.6f} , {:10.6f}] ".format(perc[j]*100, pc[0], pc[1])
print(l)
...
l = "{:12s}: MAP {:10.6f} ".format("M_c", map_Mc_Me_repar)
for j, pc in enumerate(credint):
    l += "HDI@{:.2f}% [{:10.6f} , {:10.6f}] ".format(perc[j]*100, pc[0], pc[1])
print(l)
